...
// The library brings no TLS crate; the handshake worker drives
// `truenas_ktls` — the packaged accept side: a blocking handshake that
// installs kernel TLS and refuses the connection unless the readback shows
// it engaged both directions — and the client is OpenSSL, exactly the split
// a real consumer implements. Skips when the kernel lacks the `tls` ULP, or
// when libssl cannot engage kTLS at all ([`ktls_engages`] — Ubuntu ships
