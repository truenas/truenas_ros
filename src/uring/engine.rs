...
/// Longest chain [`Engine::stage_chain`] will stage. Sized to the deepest
/// shape that has a use: a durable publish tail —
/// `write → fsync → link → rename`. A cap rather than an open-ended count
/// because every link is one more SQE that must be staged contiguously, and a
/// chain that cannot fit the SQ is an `EBUSY` the caller has to unwind.
