...
impl<U> Reactor<U> {
    ...
    pub(crate) fn close_conn(
        ...
        // Same for the total-receipt budget, and for the parked read's
        // pool-shortage retry - the only one of the three armed by default,
        // so the one most often pending here - and for the same reason.
        #[cfg(feature = "net-server")]
        {
            self.cancel_receipt_deadline(slot, generation)?;
            self.cancel_recv_retry(slot, generation)?;
        }
