...
/// The one block of state shared (`Arc`) between the loop thread and every
/// cross-thread handle — [`ShutdownHandle`]s, outstanding [`Deferred`]s /
/// [`AcceptDeferral`]s, [`PushHandle`]s: the stop/graceful-drain flags and
/// the wake eventfd their pokes ride on.
#[derive(Debug)]
...
impl WakeHandle {
    ...
    pub(crate) fn poke(&self) {
        ...
        // the server is gone (so the wake is moot).
        unsafe {
            libc::write(
                self.fd.as_raw_fd(),
