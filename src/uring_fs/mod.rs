...
mod tests {
    ...
    fn privileged_xattrs_matches_only_listed_prefixes() {
        ...
        assert!(
            !empty.permits(c"trusted.truenas_s3_etag"),
            "an empty policy grants nothing"
        );

        let p = PrivilegedXattrs::new()
            .allow_prefix(c"trusted.truenas_s3_")
            .expect("valid prefix");
        assert!(p.permits(c"trusted.truenas_s3_etag"));
        assert!(p.permits(c"trusted.truenas_s3_"), "the prefix itself");
        // Neighbouring names that merely look similar are not covered.
        assert!(!p.permits(c"trusted.truenas_s3"), "one byte short");
        assert!(!p.permits(c"trusted.other"));
        assert!(!p.permits(c"security.capability"));
        assert!(!p.permits(c"user.trusted.truenas_s3_etag"));
        // A prefix match is anchored at the start, never a substring search.
        assert!(!p.permits(c"x.trusted.truenas_s3_etag"));
    }
