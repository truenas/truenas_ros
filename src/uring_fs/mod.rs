...
//! [`unlinkat`](FsHandle::unlinkat), [`mkdirat`](FsHandle::mkdirat), ...),
//! where the anchor dirfd is what the name refers to.
//!
...
// Lives in the engine (`uring::personality`): registration is engine-level
// (`register_personality` takes a bare ring fd) and the net client stamps it
// on outbound connects, so the type cannot be fs-owned.
